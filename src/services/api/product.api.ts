...
export class ApiProductService implements IProductService {
  ...
  async getAll(params?: ProductListParams): Promise<PaginatedResponse<Product>> {
    return fetchJson(this.buildUrl({ page: params?.page, page_size: params?.page_size, category: params?.category, subcategory: params?.subcategory, sort_by: params?.sort_by, search: params?.search, min_price: params?.min_price, max_price: params?.max_price, badges: params?.badges?.join(",") }));
  }
