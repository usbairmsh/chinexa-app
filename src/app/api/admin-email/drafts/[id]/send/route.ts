...
import { getDraft, getMailbox, getThread, recordOutbound, recordBroadcast, deleteDraft } from "@/lib/email-inbox";
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (draft.kind === "reply") {
    ...
    const result = await sendEmail({ to, subject: draft.subject, html, text, from: fromLine });
    if (!result.success) return NextResponse.json({ error: result.error || "Could not send" }, { status: 502 });

    // Attach to the originating thread if it still exists.
  ...
  for (let i = 0; i < recipients.length; i += CONCURRENCY) {
    ...
    const results = await Promise.all(batch.map((to) => sendEmail({ to, subject: draft.subject, html, text, from: fromLine })));
